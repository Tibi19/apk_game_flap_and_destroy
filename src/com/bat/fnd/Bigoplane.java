...
public class Bigoplane extends Misc {
	
	private float force;
	private final boolean flipped = Util.random.nextBoolean();
	private final int index = MobileBlockManager.BIGOPLANE;
	
	public Bigoplane(World world) {
		this.world = world;
		if (flipped) 
		{
			frame = MobileBlockManager.bigoplaneTextureFlipped;
			fixtureDef = MobileBlockManager.bigoplaneFixtureDefFlipped;
		}
		else 
		{
